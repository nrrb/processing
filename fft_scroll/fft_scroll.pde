...
{
  size(512, 512, P3D);
  mm = new MovieMaker(this, width, height, "sample.mov", 30, MovieMaker.H263, MovieMaker.HIGH);
  strokeWeight(1);
...
//      int box_height = int(band_multiplier*log(this_band[j]));
      float box_height = band_multiplier * log(this_band[j]);
      if(box_height > max_height) {
        max_height = box_height;
        println(max_height);
      }
      stroke(color(map(box_height, 0, 500, 0, 0.5), 1.0, 1.0, 0.25));
      if(box_height > 0.99) {
        pushMatrix();
        translate(0, -box_height/2, 0);
//        box(box_width, box_height, box_depth);
        line(0, 0, 0, 0, -box_height, 0);
        line(0, 0, 0, box_width, 0, 0);
        line(0, 0, 0, 0, 0, -box_depth);
        popMatrix();
      }
    }
    popMatrix();
